...
SELECT  
    COUNT(CASE WHEN GENDER = 'Female' THEN 1 END) AS FEMALE_PASSENGERS FROM airline ;

-- 2. query to find the total number of passengers who have made bookings for air tickets
SELECT
    COUNT(CASE WHEN CLASS = 'Business' THEN 1 END) AS BUSINESS_CLASS_COUNT FROM airline;

-- 3. query to find number of economy class passengers
SELECT
    COUNT(CASE WHEN CLASS = 'Eco' THEN 1 END) AS ECONOMY_CLASS_COUNT FROM airline;

-- 4. query to find the number of children who are all booked tickets
SELECT
    COUNT(CASE WHEN AGE < 13 THEN 1 END) AS CHILDREN_COUNT FROM airline;

-- 5. query to find count of passengers who are above the age of 16 and have booked tickets in the business class
SELECT
    COUNT(CASE WHEN AGE > 16 AND CLASS = 'Business' THEN 1 END) AS BUSINESS_CLASS_PASSENGERS_ABOVE_16 FROM airline;
